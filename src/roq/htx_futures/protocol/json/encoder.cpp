...
std::string_view Encoder::create_order(
  ...
  buffer.clear();
  auto direction = map(create_order.side).template get<protocol::json::Direction>();
  auto order_price_type =
      map(create_order.order_type, create_order.time_in_force, create_order.execution_instructions).template get<protocol::json::OrderPriceType>();
  fmt::format_to(
  ...
  if (create_order.position_effect != PositionEffect{}) {
    auto offset = map(create_order.position_effect).template get<protocol::json::Offset>();
