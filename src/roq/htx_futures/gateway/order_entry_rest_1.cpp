...
void OrderEntryREST1::operator()(metrics::Writer &writer) const {
  writer
      // counter
...
uint16_t OrderEntryREST1::operator()(Event<CancelAllOrders> const &event, std::string_view const &request_id) {
...
void OrderEntryREST1::operator()(Trace<web::rest::Client::Connected> const &) {
  download_.begin();
}

void OrderEntryREST1::operator()(Trace<web::rest::Client::Disconnected> const &) {
  ++counter_.disconnect;
  (*this)(ConnectionStatus::DISCONNECTED);
  download_.reset();
}

void OrderEntryREST1::operator()(Trace<web::rest::Client::Latency> const &event) {
...
void OrderEntryREST1::open_orders() {
...
void OrderEntryREST1::operator()(Trace<protocol::json::OpenOrdersAck> const &event) {
  auto &[trace_info, open_orders_ack] = event;
  log::info<2>("open_orders_ack={}"sv, open_orders_ack);
  log::warn("open_orders_ack={}"sv, open_orders_ack);
...
void OrderEntryREST1::create_order(
    Event<CreateOrder> const &event, server::oms::Order const &order, server::oms::RefData const &ref_data, std::string_view const &request_id) {
...
void OrderEntryREST1::create_order_ack(Trace<web::rest::Response> const &event, uint8_t user_id, uint64_t order_id, uint32_t version) {
...
void OrderEntryREST1::cancel_order(
    Event<CancelOrder> const &event,
    server::oms::Order const &order,
    server::oms::RefData const &ref_data,
    std::string_view const &request_id,
...
void OrderEntryREST1::cancel_order_ack(Trace<web::rest::Response> const &event, uint8_t user_id, uint64_t order_id, uint32_t version) {
...
void OrderEntryREST1::cancel_all_orders(Event<CancelAllOrders> const &event, std::string_view const &request_id) {
...
void OrderEntryREST1::operator()(Trace<protocol::json::CancelAllOrdersAck> const &event) {
...
void OrderEntryREST1::process_response(web::rest::Response const &response, auto error_handler, auto success_handler) {
  try {
