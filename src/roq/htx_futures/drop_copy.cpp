...
void DropCopy::operator()(Trace<json::Close2> const &) {
  profile_.close([&]() {
    ...
    for (auto &item : accounts.data) {
      for (auto &item_2 : item.contract_detail) {
        auto funds_update = FundsUpdate{
            .stream_id = stream_id_,
            .account = account_.name,
            .currency = item_2.symbol,
            .margin_mode = {},  // XXX FIXME TODO
            .balance = item_2.margin_available,
            .hold = NaN,
            .borrowed = NaN,
            .external_account = {},
            .update_type = update_type,
...
void DropCopy::operator()(Trace<json::PositionsCross> const &event) {
  profile_.positions([&]() {
    auto &[trace_info, positions] = event;
    auto update_type = map(positions.event).template get<UpdateType>();
    for (auto &item : positions.data) {
