...
template <typename T>
struct Parser5Tester final : public protocol::json::Parser5::Handler {
  ...
  static void dispatch(callback_type const &callback, std::string_view const &message, size_t buffer_size, size_t max_depth) {
    ...
    // parser
    // XXX FIXME TODO catch2 block ???
    Parser5Tester handler{callback};
    auto res = protocol::json::Parser5::dispatch(handler, message, buffers, {}, false);
    CHECK(res == true);
    CHECK(handler.found_ == true);
  }

 protected:
  explicit Parser5Tester(callback_type const &callback) : callback_{callback} {}
  ...
  void operator()(Trace<protocol::json::Response5> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Account5> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Positions5> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::MatchOrders5> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Orders5> const &event) override { dispatch(event); }
